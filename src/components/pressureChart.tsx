...
export function PressureChart({
  ...
  const option = {
    ...
    annotations: {
      yaxis: [
        {
          y: n_data['pressure'].slice(zeroIndex).reduce(aryMin),
          borderColor: '#00E396',
          label: {
            ...
            text: n_data['pressure'].slice(zeroIndex).reduce(aryMin).toString(),
          },
        },
        {
          y: n_data['pressure'].slice(zeroIndex).reduce(aryMax),
          borderColor: '#00E396',
          label: {
            borderColor: 'transparent',
            style: {
            ...
            text: n_data['pressure'].slice(zeroIndex).reduce(aryMax).toString(),
            ofsetY: 10,
            ofsetX: n_data['time'].indexOf(n_data['pressure'].slice(zeroIndex).reduce(aryMax)),
          },
        },
        // {
      ...
      points: [
        {
          x: n_data['time'][
            n_data['pressure']
              .slice(zeroIndex)
              .indexOf(n_data['pressure'].slice(zeroIndex).reduce(aryMax)) + zeroIndex
          ],
          y: n_data['pressure'].slice(zeroIndex).reduce(aryMax),
          marker: {
            size: 4,
          ...
          label: {
            ...
            text: 'Max ' + n_data['pressure'].slice(zeroIndex).reduce(aryMax).toString() + 'hPa',
          },
        },
      ],
    },
